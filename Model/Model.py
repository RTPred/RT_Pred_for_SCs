...
# def model_input(Test_size, Num_workers, Batch_size, Shuffle):
#     f,rt = gf.Calculate_Descriptors()
#     X, y = np.array(f), np.array(rt)
...
#     train_loader = DataLoader(dataset=train_set, num_workers=Num_workers, batch_size=Batch_size, shuffle=Shuffle)
#     test_set = Data.TensorDataset(X_test, y_test)
#     test_loader = DataLoader(dataset=test_set, num_workers=Num_workers, batch_size=Batch_size, shuffle=Shuffle)
#
...
#         X_train = self.layer1(X_train)
#         X_train = self.layer2(X_train)
#         X_train = self.layer3(X_train)
#         return X_train
#
#
# def train_DNN_model(N_hidden1, N_hidden2,LR, Epoch, logger):
#     net = Net(200, N_hidden1, N_hidden2, 1)
#     optimize = optim.SGD(net.parameters(), lr=LR)
#     loss_func = torch.nn.MSELoss()
#
#     losses = []
#     MAE = []
#     tr, tes = model_input(Test_size=0.2, Num_workers=20, Batch_size=15, Shuffle=True)
#     print(f"Len(tr): {len(tr)}")
#     for eph in range(Epoch):
#         train_loss = 0
#         r2 = 0
#         mae = 0
#         for tar, tar_label in tr:
#             tar = Variable(tar)
#             tar_label = Variable(tar_label)
#
#             output = net(tar)
...
#             r2_sc = r2_score(tar_label.data.numpy(), output.data.numpy())
#             r2 += r2_sc
#             Mae = mean_absolute_error(tar_label.data.numpy(), output.data.numpy())
#             mae += Mae
#
#   #      losses.append(train_loss/len(tr))
#
#         if (eph+1) % 10 == 0:
#             logger.info(f"Epoch:{eph+1}/{Epoch} done. loss: {train_loss / len(tr)},MAE: {mae/len(tr)}")
# #            print(f"Epoch: {eph+1}, loss: {train_loss / len(tr)},"   #r2_score:{r2 / len(tr)},
# #                  f"MAE: {mae/len(tr)}")
